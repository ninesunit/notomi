      ...
      .then((groups) => {
        ...
        setRows(
          groups
            ...
            .sort(
              (a, b) =>
                (b.document.createdAt?.toMillis?.() ?? 0) -
                (a.document.createdAt?.toMillis?.() ?? 0)
            )
        );
      })
    ...
    <ScreenScroll>
      ...
      {loading ? (
        ...
        <View className="gap-2">
          {rows.map(({ subject, document }) => (
            <Pressable
              key={`${subject.id}-${document.id}`}
              ...
              <Card className="flex-row items-center gap-3 py-3.5">
                ...
                <View className="flex-1 gap-0.5">
                  ...
                  <Text className="text-xs text-muted" numberOfLines={1}>
                    {subject.name}{subject.moduleCode ? ` · ${subject.moduleCode}` : ''}
                  </Text>
                </View>
