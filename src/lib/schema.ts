...
export type Subject = {
  ...
  /**
   * The Drive folder holding this subject's originals.
   *
   * Remembered rather than looked up: Notomi asks Google for drive.file scope
   * only, and an app with that scope cannot search for a folder it did not
   * create. Absent until the student first uploads with Drive connected.
   */
...
export type SourceDocument = {
  ...
  r2FileUrl: string;
  /**
   * Set when the original lives in the student's own Drive instead of R2.
   * Both may be empty — the text is extracted on device, so a document stays
   * fully usable even when nowhere kept the original.
   */
  driveFileId?: string | null;
  /**
   * The chapter or topic this belongs to, when the student has said.
   *
   * Overrides the model's guess, which is often nothing at all — a bare slide
   * deck rarely states its chapter, and everything then piles into one bucket
   * the student cannot correct.
   */
  chapter?: string | null;
  /** Manual position within its chapter; null means use the file's numbering. */
  order?: number | null;
  moduleCode: string | null;
  summary: string | null;
